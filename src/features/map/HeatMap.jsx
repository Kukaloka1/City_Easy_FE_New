...
export default function HeatMap({ incidents = [], center = DEFAULT_CENTER, zoom = 11 }) {
  const containerRef = useRef(null)
  const mapRef = useRef(null)
  const heatLayersRef = useRef([])
  const [ready, setReady] = useState(false)

  // Agrupa solo últimos 24h por tipo
  ...
  // 2) Refresca capas cuando cambian los datos
  useEffect(() => {
  ...
  return (
    <div
      ref={containerRef}
      className="relative h-[420px] w-full overflow-hidden rounded-xl border"
    />
  )
}
